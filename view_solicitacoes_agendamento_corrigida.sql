...
SELECT 
  ...
  -- Endereço completo da unidade (concatenando campos existentes)
  CASE 
    WHEN un.endereco IS NOT NULL AND un.numero IS NOT NULL AND un.bairro IS NOT NULL AND un.cidade IS NOT NULL AND un.estado IS NOT NULL THEN
      CONCAT(un.endereco, ', ', un.numero, ' - ', un.bairro, ', ', un.cidade, ' - ', un.estado)
    WHEN un.endereco IS NOT NULL AND un.cidade IS NOT NULL THEN
      CONCAT(un.endereco, ', ', un.cidade, ' - ', un.estado)
    WHEN un.endereco IS NOT NULL THEN
      un.endereco
    ELSE
      'Endereço não informado'
  END as endereco_completo_unidade,
...
RETURNS TABLE (
  ...
  endereco_completo_unidade text,
  prioridade varchar,
